    ...
    logical, parameter :: second_order_tightcoupling = .false. !KZ
    ...
    do nu_i = 1, CP%Nu_mass_eigenstates
        ...
        if (EV%MassiveNuApprox(nu_i)) then
           ...
           ! write(*,*) ' this is true ', 1/a-1, nu_i
        else
            !write(*,*) ' this is false ', 1/a-1, nu_i
            !Integrate over q
            call Nu_Integrate_L012(EV, y, a, nu_i, clxnu,qnu,dpnu,pinu)
            !clxnu_here  = rhonu*clxnu, qnu_here = qnu*rhonu
            !dpnu=dpnu/rhonu
            ...
            !write(*,*) pinudot, yprime(EV%nu_ix(nu_i)+3), (pinudot-yprime(EV%nu_ix(nu_i)+3))/pinudot
        endif
    ...
    subroutine Nu_Integrate_L012V2(EV, y, a, nu_i, dpnu, pinu)
        type(EvolutionVars) EV
        integer, intent(in) :: nu_i
        real(dl), intent(in) :: a, y(EV%nvar)
        real(dl), intent(OUT) :: dpnu, pinu
        real(dl) tmp, am, aq,v, pert_scale, drhonu, fnu
        integer iq, ind
        ...
        dpnu = dpnu/3
    end subroutine Nu_Integrate_L012V2
    ...
    real(dl) dgrho_de, dgq_de, cs2_de, dgpi_de, sigma_ppf !KZ begin
    real(dl) gpnu_t
    real(dl) grhormass_t, rhonudot
    real(dl) qnu, dpnu, grhonu, dgrhonu, pinua! KZ
    real(dl) rhonu, pnu, pinu, pinudot, dgpi_nu_sum, ppi_nu_dot_sum !KZ
    ...
    !KZ-edit begin: move DE calculation after Neutrino calculations


    pinu = 0
    pinudot = 0
    gpnu_t = gpres_nu !rename

    !KZ-edit end

    !VM HERE IS WHERE WE COMPUTE SIGMA_PPF
    !For now just use this function, should be a seperate function in the future

    !======The function to get sigma_ppf=====

    !VM HERE IS WHERE WE COMPUTE NEUTRINO PI AND PI_TOTAL
    !Use the same method as in MassiveNuVarsOut
    dgpi_nu_sum = 0.0
    ppi_nu_dot_sum = 0.0
    do nu_i = 1, CP%Nu_mass_eigenstates
        call ThermalNuBack%rho_P(a*State%nu_masses(nu_i), rhonu, pnu)
        if (EV%MassiveNuApprox(nu_i)) then
            pinu = ay(EV%nu_ix(nu_i) + 3)
            pinudot = ayprime(EV%nu_ix(nu_i) + 3)
        else
            call Nu_Integrate_L012V2(EV, ay, a, nu_i, dpnu, pinu)
            pinu = pinu/rhonu
            call Nu_pinudot(EV, ay, ayprime, a, adotoa, nu_i, pinudot)
            pinudot = pinudot/rhonu - ThermalNuBack%drho(a*State%nu_masses(nu_i),adotoa)/rhonu*pinu
        endif
        dgpi_nu_sum = dgpi_nu_sum  + ((State%grhormass(nu_i)/a**2)*rhonu)*pinu
        ppi_nu_dot_sum = ppi_nu_dot_sum + ((State%grhormass(nu_i)/a**2)*rhonu)*pinudot - 3._dl * (((State%grhormass(nu_i)/a**2)*rhonu)+State%grhormass(nu_i)/a**2*pnu)*adotoa*pinu
    end do
    !VM HERE WE COMPUTE PI_T AND PI_DOT_T (THESE ARE THE ONLY VARIABLES WE SHOULD SEND TO PerturbedStressEnergy)

    !VM: HERE IS WHERE WE CALCULATE DARK ENERGY PERTURBATIVE FUNCTIONS
    if (.not. EV%is_cosmological_constant) then
        call State%CP%DarkEnergy%PerturbedStressEnergy(dgrho_de, dgq_de, dgpi_de, sigma_ppf, &
            a, dgq, dgrho, grho, grhov_t, w_dark_energy_t, gpres_noDE, etak, &
            adotoa, k, EV%Kf(1), ay, ayprime, EV%w_ix, grhor_t, grhog_t, pir, pig, pirdot, pigdot, grhonu_t, gpnu_t,dgpi_nu_sum, ppi_nu_dot_sum)
        dgrho = dgrho + dgrho_de
        dgq = dgq + dgq_de
        dgpi = dgpi_de ! Add AS contribution from DE
    end if

    !VM: HERE IS WHERE WE COMPUTE Z and SIGMA WITH THE STANDARD FORMULA
    if (State%flat) then
       ...
       write(*,*) ' WE ARE NOT WORKING WITH CURVATURE'
       STOP
    end if
    ...
    ! ------------------------------------------------------------------------------------
    !VM WARINING
    ! THIS IS THEIR OLD OUTPUT FUNCTION
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        !KZ-edit BEGINS
        ! We want to extract the neutrino contribution
        ! MassiveNuVarsOut contains the line pidot_sum = pidot_sum + grhonu_t*pinudot
        ! MassiveNuVarsOut constains the line dgpi = dgpi + grhonu_t*pinu
        clxnu =0
        ...
        !pinudot = pidot_sum - pinudot
        !pinu    = (dgpi - pinu)/grhonu_t !Subtract back to get contribution from neutrino

        !gpnu_t = gpres_nu ! rename
        !then calculation DE perturbation
        !if (.not. EV%is_cosmological_constant) then
        !    call State%CP%DarkEnergy%PerturbedStressEnergy(dgrho_de, dgq_de, &
        !        a, dgq, dgrho, grho, grhov_t, w_dark_energy_t, gpres_noDE, etak, &
        !        adotoa, k, EV%Kf(1), ay, ayprime, EV%w_ix, grhor_t, grhog_t, pir, pig, pirdot, pigdot, grhonu_t, pinudot, gpnu_t, pinu)
        !    dgrho = dgrho + dgrho_de
        !    dgq = dgq + dgq_de
        !    end if
        !KZ: ENDS 
        ...
        gpres = gpres_noDE + w_dark_energy_t*grhov_t
        diff_rhopi = pidot_sum - (4*(dgpi-dgpi_de)+ dgpi_diff)*adotoa + & !KZ-edit take dtpi_de out
            State%CP%DarkEnergy%diff_rhopi_Add_Term(dgrho_de, dgq_de, grho, &
            gpres, w_dark_energy_t, State%grhok, adotoa, &
            EV%kf(1), k, grhov_t, z, k2, ayprime, ay, EV%w_ix, dgpi_de)
