...
10      if (this%CP%Omnuh2 < 1.e-7_dl) this%CP%Omnuh2 = 0
    ...
    real(dl), intent(out) :: densities(7+max_num_of_fluids,n)
    integer nu_i,i
    ...
    do i=1, n
        ...
        densities(8:8+max_num_of_fluids-1,i) = grhov_t*a**2
        densities(1,i) = sum(densities(2:8+this%cp%DarkEnergy%num_of_components-1,i))
    end do
    ...
    real(dl), dimension(max_num_of_fluids, n), intent(out) :: grhov_t, w
    integer :: i, j
    ! JVR - Modified to return an array for each component
    do i=1, n
        call this%CP%DarkEnergy%BackgroundDensityAndPressure(1._dl, a(i), grhov_t(1:max_num_of_fluids, i), w(1:max_num_of_fluids, i))
        grhov_t(1:this%cp%DarkEnergy%num_of_components, i) = grhov_t(1:this%cp%DarkEnergy%num_of_components, i) / (a(i)**2)
    end do
